...
void print_to_a_file(const char *filepath, const char *message) {
    FILE *fp = fopen(filepath, "w");                // Open in write mode
    if (!fp) {
        perror("fopen");                             // Print why fopen failed
        return;
    }

    if (fprintf(fp, "%s", message) < 0) {            // Write the message
        perror("fprintf");                           // If write fails, report
        fclose(fp);
        return;
    }

    if (fclose(fp) != 0) {                           // Close and flush
        perror("fclose");
    }
}
...
/**
 * Send “message\n” to the local server, read back one line of response,
 * parse 7 comma‐separated doubles, return malloc()’d double[7].
 * On any error, return malloc()’d array of length=1 with value -1.0.
 * Also set *out_count to either 7 (success) or 1 (error) or 0 (malloc failed).
 */
double *send_and_receive(const char *message, int *out_count) {
    *out_count = 0;
    int sock = -1;
    ...
    if (message[ml - 1] != '\n') {
        ...
        memcpy(sendbuf, message, ml);
        sendbuf[ml]     = '\n';
        sendbuf[ml + 1] = '\0';
    } else {
        sendbuf = strdup(message);
        if (!sendbuf) {
            perror("strdup");
            goto ERR;
        }
    ...
    // 3) read exactly one line (up to '\n')
    ssize_t total = 0;
    while (1) {
        ssize_t n = read(sock, recvbuf + total, 1);
        if (n < 0) {
            perror("read");
            goto ERR;
        } else if (n == 0) {
            fprintf(stderr, "Connection closed before newline\n");
            goto ERR;
        }
        if (recvbuf[total] == '\n') {
            total++;
        ...
        total++;
        if (total >= (ssize_t)(sizeof(recvbuf) - 1)) {
    ...
    // 4) parse 7 comma‐separated doubles
    const int EXPECTED = 7;
    double *vals = malloc(sizeof(double) * EXPECTED);
    if (!vals) {
        perror("malloc");
        return NULL;
    }
    char *tmp = strdup(recvbuf);
    if (!tmp) {
        perror("strdup");
        free(vals);
        return NULL;
    ...
    int count = 0;
    for (char *tok = strtok(tmp, ","); tok && count < EXPECTED; tok = strtok(NULL, ",")) {
    ...
    if (count != EXPECTED) {
        ...
        free(vals);
        double *err = malloc(sizeof(double) * 1);
        if (!err) return NULL;
        err[0] = -1.0;
        *out_count = 1;
        return err;
                              ...
                              double *var)
{
    ...
    char command_vmf3[256];
    snprintf(command_vmf3, sizeof(command_vmf3),
             "%ld,%.6f,%.6f,%.6f",  // note: use long format if time.time is long
             (long)time.time, 0.0, azel[0], azel[1]);
    print_to_a_file("/home/kaue/RTKLIB/response.txt", "2a\n");

    int nvals = 0;
    double *values = send_and_receive(command_vmf3, &nvals);
    if (!values || (nvals == 1 && values[0] < 0.0)) {
        // error, so free and return zero delay
        free(values);
        return 0.0;
    }

    // Build a debug string with all seven values:
    char debugstr[200];
    int len = snprintf(debugstr, sizeof(debugstr),
                       "5,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f\n",
                       values[0], values[1], values[2],
                       values[3], values[4], values[5], values[6]);
    if (len < 0 || (size_t)len >= sizeof(debugstr)) {
        strcpy(debugstr, "Format error\n");
    }
    print_to_a_file("/home/kaue/RTKLIB/response.txt", debugstr);
