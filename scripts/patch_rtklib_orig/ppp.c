...
/* Function prototype for 'close_fout' */
void close_fout(void);
                       ...
                       double *var)
{
    ...
    static char fileText[200];
    static int fileTextInitialized = 0;
    static int foutInitialized = 0;

    double zhd = 0.0, m_h = 0.0, m_w = 0.0, cotz = 0.0, grad_n = 0.0, grad_e = 0.0;
...
if ((opt->tropopt == TROPOPT_ESTG || opt->tropopt == TROPOPT_CORG) && azel[1] > 0.0)
    {
        ...
        /* printf("cotz: %f, grad_n: %f, grad_e: %f\n", cotz, grad_n, grad_e); */
    ...
    *var = SQR(0.01);
    ...
    /* Initialize fileText only once */
    if (!fileTextInitialized)
    {
        FILE *infileStream = fopen("/home/RTKLIB/curr_delaypath.txt", "r");
        if (infileStream != NULL)
        {
            if (fgets(fileText, sizeof(fileText), infileStream) == NULL)
            {
                fprintf(stderr, "Error reading fileText\n");
                fclose(infileStream);
        ...
        {
            perror("Error opening curr_delaypath.txt");
            return -1; /* Handle error appropriately */
        }
    }
    ...
    if (!foutInitialized)
    {
        ...
        /* Register function to close the file at program exit */
        if (atexit(close_fout) != 0)
        {
            fprintf(stderr, "Error registering close_fout with atexit()\n");
            return -1; /* Handle error appropriately */
        }
    }
...
/* Function to close the output file at program exit */
void close_fout(void)
{
    if (fout != NULL)
    {
